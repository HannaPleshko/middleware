...
/* This was originally from the package ical.js by Peter Braden: https://github.com/peterbraden/ical.js */
/* We pulled it into our project and created quattro-ical from it to modify the parsing to include the COMMENT field. */
...
const storeParameter = function (name, parameters, curr) {
  return function (value, iParameters, iCurr) {
    const data = iParameters && iParameters.length > 0 && !(iParameters.length === 1 && iParameters[0] === 'CHARSET=utf-8') ? {params: parseParameters(iParameters), val: text(value)} : text(value);

    return storeValueParameter(name)(data, iCurr);
  };
};
...
const dateParameter = function (name) {
  return function (value, parameters, curr) {
    ...
    if (parameters && parameters.includes('VALUE=DATE') && !parameters.includes('VALUE=DATE-TIME')) {
      ...
      const icalcomps = /^(\d{4})(\d{2})(\d{2}).*$/.exec(value);
      if (icalcomps !== null) {
        // No TZ info - assume same timezone as this computer
        newDate = new Date(icalcomps[1], Number.parseInt(icalcomps[2], 10) - 1, icalcomps[3]);
...
const freebusyParameter = function (funname) {
  return function (value, parameters, curr) {
    const fb = addFBType({}, parameters);
    curr[funname] = curr[funname] || [];
    curr[funname].push(fb);

    storeParameter(value, parameters, fb);

    const parts = value.split('/');

    ['start', 'end'].forEach((fname, index) => {
      dateParameter(fname)(parts[index], parameters, fb);
    });
...
module.exports = {
  objectHandlers: {
    ...
    END(value, parameters, curr, stack) {
      ...
      const originalEnd = function (component, parameters_, iCurr, iStack) {
        ...
        if (component === 'VCALENDAR') {
          ...
          for (key in iCurr) {
            if (!{}.hasOwnProperty.call(iCurr, key)) {
              continue;
            }
        ...
        const par = iStack.pop();

        if (iCurr.uid) {
          // If this is the first time we run into this UID, just save it.
          if (par[iCurr.uid] === undefined) {
            par[iCurr.uid] = curr;
          } else if (iCurr.recurrenceid === undefined) {
          ...
          if (typeof iCurr.recurrenceid !== 'undefined') {
            ...
            for (key in iCurr) {
              if (key !== null) {
                recurrenceObject[key] = iCurr[key];
              }
            }
            ...
            if (par[iCurr.uid].recurrences === undefined) {
              par[iCurr.uid].recurrences = {};
            }
            ...
            if (typeof iCurr.recurrenceid.toISOString === 'function') {
              par[iCurr.uid].recurrences[curr.recurrenceid.toISOString().slice(0, 10)] = recurrenceObject;
            } else {
              console.error('No toISOString function in curr.recurrenceid', iCurr.recurrenceid);
            }
          }

          // One more specific fix - in the case that an RRULE entry shows up after a RECURRENCE-ID entry,
          // let's make sure to clear the recurrenceid off the parent field.
          if (typeof par[iCurr.uid].rrule !== 'undefined' && typeof par[iCurr.uid].recurrenceid !== 'undefined') {
            delete par[iCurr.uid].recurrenceid;
          }
        } else {
          par[uuid()] = iCurr;
        }
    ...
    COMMENT: storeParameter('comment'),
